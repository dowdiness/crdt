...
- [ ] **Term enum extensibility** — Term (10 variants, 24 match sites) is the biggest Expression Problem hotspot. Two-layer architecture in progress: (a) `TermSym` Finally Tagless trait + `replay` function (pending, loom submodule — see `docs/plans/2026-03-28-two-layer-architecture-impl.md`); (b) framework extraction to decouple `@ast`-free framework packages from lambda-specific code (Phase 1 ✅ done — PR #60; Phase 2 ✅ done — PR #62; Phase 3 partially done — PR #66: `framework/core/` with `NodeId`, `ProjNode[T]`, helpers; traits/SourceMap/reconcile stay in projection/ due to orphan rule; Phases 4–5 in progress — see `docs/plans/2026-03-28-framework-extraction-impl.md`).
