...
- [ ] Consolidate the markdown ZWSP empty-paragraph sentinel behind a single MoonBit constant + a moji Unicode-property predicate.
  Why: the U+200B sentinel literal is hardcoded six times across MoonBit and TypeScript (3 sites in `lang/markdown/`, 3 in `adapters/editor-adapter/`) with no shared definition. moji already owns Unicode-property work but lacks the `Default_Ignorable_Code_Point` predicate that would let the sentinel rule generalize cleanly if needed.
  Plan: `docs/plans/2026-05-16-moji-zwsp-consolidation.md`
  Exit: `git grep '​' -- '*.mbt' '*.ts' '*.tsx'` returns only the two named constants (one MoonBit, one TS) and annotated test fixtures; moji exports `is_default_ignorable_code_point`.
